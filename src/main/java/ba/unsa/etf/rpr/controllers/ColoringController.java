...
/**
 *ColoringController class is responsible for scheduling an appointment for hair coloring.
...
public class ColoringController{
    ...
    public ColoringController(){
        services.add("Highlights (on all lengths)");
        services.add("Balayage (on all lengths)");
        services.add("Ombre (on all lengths)");
        services.add("Single process (on all lengths)");
    }
    /**
    ...
    public void confirmButtonOnAction(ActionEvent actionEvent) throws HairsalonException {
        if(serviceId.getValue().isEmpty() || stylistPickId.getValue()==null  || datePickerId.getValue()==null){
            ...
            if (service.contains("Highlights")) {
                appointments.setPrice("125$");
                appointments.setDuration(130);
            } else if (service.contains("Balayage")) {
                appointments.setPrice("155$");
                appointments.setDuration(145);
            } else if (service.contains("Ombre")) {
                appointments.setPrice("145$");
                appointments.setDuration(160);
            } else if (service.contains("Single")) {
